...
\noindent Para acessar e manipular arquivos e pastas, digite no terminal, substituindo \textit{nome.sobrenome} pelo usuário fornecido pelo \textit{Helpdesk}:
\bigskip
...
\begin{tcolorbox}[enhanced,
  ...
  overlay={\begin{tcbclipinterior}\fill[red!75!blue!50!white] (frame.south west)
    rectangle node[text=white,font=\sffamily\bfseries\footnotesize,rotate=0] {ATENÇÃO} ([xshift=18mm]frame.north west);\end{tcbclipinterior}}]
O COAWST versão 3.2 já está dentro do repositório discutido na Seção \textcolor{bleu_cite}{\ref{reposit}}.
\end{tcolorbox}
\bigskip
...
\noindent Abra o arquivo \textit{Config\_new.pl}
\bigskip
...
\noindent Esta modificação selecionará, na compilação, as configurações da Kerana (\textit{CRAY CCE (ftn/gcc): Cray XE and XC (dmpar)}), entre as várias disponíveis para usar o COAWST, como na Figura \textcolor{bleu_cite}{\ref{compskerana}}.
\bigskip
...
\begin{tcolorbox}[enhanced,
  ...
  overlay={\begin{tcbclipinterior}\fill[red!75!blue!50!white] (frame.south west)
    rectangle node[text=white,font=\sffamily\bfseries\footnotesize,rotate=0] {ATENÇÃO} ([xshift=18mm]frame.north west);\end{tcbclipinterior}}]
Este guia utiliza o COAWST v 3.2.
\end{tcolorbox}
\bigskip
...
\noindent Caso necessário, altere os diretórios de acordo com o nome da sua pasta do COAWST e ative o arquivo para carregar os módulos necessários:
\bigskip
...
\noindent Serão carregadas na tela as bibliotecas, conforme apresentado na Figura \textcolor{bleu_cite}{\ref{modulos}}:
\bigskip
...
\noindent Vá até a pasta do MCT:
\bigskip

\begin{bashcode}
/home/nome.sobrenome/COAWST/Lib/MCT
\end{bashcode}
\bigskip

\noindent Compile o MCT com os seguintes comandos:
\bigskip

\begin{bashcode}
./configure --prefix=/scratch/nome.sobrenome/COAWST/Lib/MCT/pgi
make
make install
\end{bashcode}
\bigskip
